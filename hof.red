...
Red [
    ...
    Name: "HOF"
    Version: 2.0.1
    Author: {Jan Skibinski (original author for Rebol) &
             noein (modified and adapted for Red)}
]
...
|                    ((number -+ number -+ number) -+ n-ring -+ n-ring -+ n-ring)
    ...
    zipwith: func [
        ...
        result: make block! size
        repeat i size [
            insert tail result f xs/:i ys/:i
        ]
        result
    ]
    ...
    require: func [
        ...
        foreach p preconditions [
            if not (do p) [
                cause-error 'user 'message rejoin ["Violated precondition " mold p]
            ]
        ]
        true
    ]
